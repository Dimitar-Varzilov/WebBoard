...
describe('TaskDetailComponent', () => {
  ...
  describe('Task Assignment Display', () => {
    it('should handle task with jobId', () => {
      ...
      expect(component.task.isAssignedToJob).toBe(true);
      expect(component.task.jobId).toBe('job-123');
    });

    it('should handle task without jobId', () => {
      if (component.task) {
        expect(component.task.isAssignedToJob).toBe(false);
        expect(component.task.jobId).toBeUndefined();
      }
    });
  });
});
