...
export const JOBS_ENDPOINTS = {
  ...
  UPDATE: (id: string) => buildEndpoint(CONTROLLERS.JOBS, `/${id}`),
  DELETE: (id: string) => buildEndpoint(CONTROLLERS.JOBS, `/${id}`),
  GET_PENDING_TASKS_COUNT: buildEndpoint(
    CONTROLLERS.JOBS,
    '/validation/pending-tasks-count'
  ),
  GET_AVAILABLE_TASKS: buildEndpoint(
    CONTROLLERS.JOBS,
    '/validation/available-tasks'
  ),
} as const;
...
export const TASKS_ENDPOINTS = {
  ...
  GET_BY_STATUS: (status: TaskItemStatus) =>
    buildEndpoint(CONTROLLERS.TASKS, `/status/${status}`),
  ...
  GET_COUNT_BY_STATUS: (status: TaskItemStatus) =>
    buildEndpoint(CONTROLLERS.TASKS, `/status/${status}/count`),
} as const;
...
export const REPORTS_ENDPOINTS = {
  ...
  DOWNLOAD: (id: string) =>
    buildEndpoint(CONTROLLERS.REPORTS, `/${id}/download`),
